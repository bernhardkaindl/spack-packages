...
3. Check the `type=` argument of the added/changed `depends_on`: When the dependency is obviously only a build dependency, the type argument should be only `type="build"`. When the dependency is likely a runtime dependency, check that type "run" is added.
4. All upper-bound version constraints are written in inclusive form.
   Confirm details against [`spack-spec-syntax`](spack-spec-syntax.md), especially when translating
   half-open upstream constraints or reviewing `when=` ranges.
